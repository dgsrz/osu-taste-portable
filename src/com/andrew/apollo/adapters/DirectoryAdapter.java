...
/**
 * This {@link android.widget.ArrayAdapter} is used to display all of the Directories on a user's
...
public class DirectoryAdapter extends ArrayAdapter<Directory> {
    ...
    /**
     * Constructor of <code>DirectoryAdapter</code>
     *
     ...
     *            load.
     */
    public DirectoryAdapter(final Activity context, final int layoutId) {
        super(context, 0);
        // Get the layout Id
        mLayoutId = layoutId;
    ...
    public void buildCache() {
        ...
        for (int i = 0; i < getCount(); i++) {
            // Build the album
            final Directory directory = getItem(i);

            // Build the data holder
            mData[i] = new DataHolder();
            // Album Id
            mData[i].mItemId = directory.mDirectoryId;
            // Album names (line one)
            mData[i].mLineOne = directory.mDirPath;
            // Album artist names (line two)
            mData[i].mLineTwo = directory.mArtistName;
            // Number of songs for each album (line three)
            mData[i].mLineThree = MusicUtils.makeLabel(getContext(),
                    R.plurals.Nsongs, directory.mSongNumber);
        }
    }
     ...
     *
     * @param album The {@link android.widget.ImageView} holding the album
     * @param position The position of the album to play.
     *
     * 功能未实现，需要重写MusicUtils
     */
    private void playAlbum(final ImageView album, final int position) {
        album.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(final View v) {
                final long id = getItem(position).mDirectoryId;
