...
import com.andrew.apollo.adapters.DirectoryAdapter;
import com.andrew.apollo.cache.ImageFetcher;
import com.andrew.apollo.loaders.DirectoryLoader;
...
/**
 * This class is used to display all of the directories on a user's device.
 * 
 * @author Andrew Neal (andrewdneal@gmail.com)
 * @author dgsrz (dgsrz@vip.qq.com)
 */
public class DirectoryFragment extends Fragment implements LoaderCallbacks<List<Directory>>,
     ...
     * Directory song list
     */
    private long[] mDirectoryList;

    /**
     * Represents an directory
     */
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        mAdapter = new DirectoryAdapter(getActivity(), layout);
    }
    ...
    public void onCreateContextMenu(final ContextMenu menu, final View v,
        ...
        // Create a new album
        mDirectory = mAdapter.getItem(info.position);
        // Create a list of the album's songs
        mDirectoryList = MusicUtils.getSongListForAlbum(getActivity(), mDirectory.mDirectoryId);
        ...
        if (item.getGroupId() == GROUP_ID) {
            switch (item.getItemId()) {
                case FragmentMenuItems.PLAY_SELECTION:
                    MusicUtils.playAll(getActivity(), mDirectoryList, 0, false);
                    return true;
                case FragmentMenuItems.ADD_TO_QUEUE:
                    MusicUtils.addToQueue(getActivity(), mDirectoryList);
                    return true;
                case FragmentMenuItems.MORE_BY_ARTIST:
                    NavUtils.openArtistProfile(getActivity(), mDirectory.mArtistName);
                    return true;
                case FragmentMenuItems.DELETE:
                    mShouldRefresh = true;
                    final String album = mDirectory.mDirPath;
                    DeleteDialog.newInstance(album, mDirectoryList,
                            ImageFetcher.generateAlbumCacheKey(album, mDirectory.mArtistName))
    ...
    public void onItemClick(final AdapterView<?> parent, final View view, final int position,
            final long id) {
        mDirectory = mAdapter.getItem(position);
        NavUtils.openDirectoryProfile(getActivity(), mDirectory.mDirPath, mDirectory.mArtistName, mDirectory.mDirectoryId);
    }
     ...
     */
    @Override
    public Loader<List<Directory>> onCreateLoader(final int id, final Bundle args) {
     ...
     */
    @Override
    public void onLoadFinished(final Loader<List<Directory>> loader, final List<Directory> data) {
        ...
        for (final Directory directory : data) {
     ...
     */
    @Override
    public void onLoaderReset(final Loader<List<Directory>> loader) {
     ...
     * header in the {@link com.viewpagerindicator.TitlePageIndicator}.
     */
    ...
    private int getItemPositionByAlbum() {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            if (mAdapter.getItem(i).mDirectoryId == albumId) {
                return i;
            }
        }
        return 0;
